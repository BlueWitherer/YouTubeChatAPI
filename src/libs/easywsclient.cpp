...
namespace { // private module-only namespace
    ...
    public:
        ...
        void poll(int timeout) { // timeout in milliseconds
            ...
            while (true) {
                ...
                if (false) {} else if (ret < 0 && (socketerrno == SOCKET_EWOULDBLOCK || socketerrno == SOCKET_EAGAIN_EINPROGRESS)) {
                    rxbuf.resize(N);
                    break;
                } else if (ret <= 0) {
                    rxbuf.resize(N);
                    closesocket(sockfd);
                    readyState = CLOSED;
                    geode::log::error("{}", ret < 0 ? "Connection error!\n" : "Connection closed!\n");
                    break;
                } else {
                    rxbuf.resize(N + ret);
                }
        ...
        virtual void _dispatchBinary(BytesCallback_Imp& callable) {
            ...
            while (true) {
                ...
                } else if (ws.opcode == wsheader_type::PONG) {} else if (ws.opcode == wsheader_type::CLOSE) { close(); } else {
        ...
        void sendData(wsheader_type::opcode_type type, uint64_t message_size, Iterator message_begin, Iterator message_end) {
            ...
            header[0] = 0x80 | type;
            if (false) {} else if (message_size < 126) {
    ...
    easywsclient::WebSocket::pointer from_url(const std::string& url, bool useMask, const std::string& origin) {
        ...
        if (false) {} else if (sscanf(url.c_str(), "ws://%[^:/]:%d/%s", host, &port, path) == 3) {
